...
const createEnquiryIntoDb = async (payload) => {
    ...
    if (existingEnquiry) {
        throw new AppError(StatusCodes.CONFLICT, "You have already submitted this enquiry. Our team is reviewing it!");
    }
    ...
    sendEmail({
        to: config.email.adminEmailAddress, // Your admin email
...
const getAllEnquiriesFromDb = async (query) => {
...
const updateEnquiryStatusIntoDb = async (enquiryId, status) => {
    const result = await Enquiry.findByIdAndUpdate(enquiryId, { status }, { new: true, runValidators: true });
    if (!result)
        throw new AppError(StatusCodes.NOT_FOUND, "Enquiry not found");
    return result;
};
const deleteEnquiryFromDb = async (enquiryId) => {
    const result = await Enquiry.findByIdAndDelete(enquiryId);
    if (!result)
        throw new AppError(StatusCodes.NOT_FOUND, "Enquiry not found");
    return result;
};
const getSingleEnquiryFromDb = async (enquiryId) => {
