...
const loginUser = async (payload) => {
    ...
    const accessToken = createToken(jwtPayload, config.JWT_SECRET, config.JWT_EXPIRES_IN);
    // 2. Create Refresh Token
    const refreshToken = createToken(jwtPayload, config.refreshTokenSecret, config.jwtRefreshTokenExpiresIn);
    return {
        accessToken,
        refreshToken,
        user: {
...
const forgotPassword = async (email) => {
    ...
    try {
        ...
        throw new AppError(StatusCodes.INTERNAL_SERVER_ERROR, `Failed to send verification email: ${error.message}`);
    }
    return null;
};
// STEP 2: Verify OTP
const verifyOTP = async (email, otp) => {
    ...
    const accessToken = createToken({ email: user.email, role: user.role }, config.JWT_SECRET, config.JWT_EXPIRES_IN);
...
const resetPasswordFromDB = async (accessToken, payload) => {
    ...
    const isSamePassword = await User.isPasswordMatched(newPassword, user.password);
    if (isSamePassword) {
